    ...
    ele.forEach(function (v, i, a) {
        if (i > 0) {
            if (v[0] - a[i - 1][0] < -180) {//west to east
                ...
                direction = 1;
                //a[i-1][0] less than 0 so ...
                /*
                const delta_lon = v[0] - 360 - a[i - 1][0];
                const delta_lat = v[1] - a[i - 1][1];
                lat_on_180 = v[1] - (-180 + v[0]) * delta_lat / delta_lon;
*/
                cursor = devideArray(cursor, direction, result, v, i, a);
            } else {
                //       alert("hey #8:(" + v[0] + ", " + v[1] + ")");
            ...
            for (let k = cursor; k < ele.length; k++) {
                tempArray2.push(ele[k]);
            }
